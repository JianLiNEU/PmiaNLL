...
#     """
#     def __init__(self, n_components, n_features, covariance_type="full", eps=1.e-6, init_params="kmeans", mu_init=None, var_init=None,device="cuda:0"):
#         """
...
#         self.mu_init = mu_init #初始的mu
#         self.var_init = var_init #初始的var
#         self.eps = eps
...
#         print(" the mu value of the first distribution: ", self.mu)
#         # print(" the mu value of the second distribution: ",self.mu[1])
...
#             mu = self.mu.to(self.device)
#             prec = torch.rsqrt(self.var).to(self.device)
...
#         weighted_log_prob = self._estimate_log_prob(x) + torch.log(self.pi).to(self.device)
...
#             x:          torch.Tensor (n, 1, d)
#         """
#         _, log_resp = self._e_step(x) # get the log-responsibilities
#         pi, mu, var = self._m_step(x, log_resp) # get the mu and var based on the log-responsibilities
...
#         """
#         # obtain the posterior predict of the single distribution
#         x represent the value of input feauture
#         y represent the posteries of specific distribution (0 or 1)

#         """
#         print("x",x)
#         sample_post_value=self.predict(x, probs=True)
#         print(sample_post_value)
#         return sample_post_value[:,y:y+1]
...
class GaussianMixture(torch.nn.Module):
    ...
    def _estimate_log_prob(self, x):
        ...
        if self.covariance_type == "full":
            ...
            x_mu_T_precision = calculate_matmul_n_times(self.n_components, x_mu_T, precision).to(x.device)
            x_mu_T_precision_x_mu = calculate_matmul(x_mu_T_precision, x_mu).to(x.device)

            return -.5 * (log_2pi- log_det.to(x.device) + x_mu_T_precision_x_mu.to(x.device))
    ...
    def _e_step(self, x):
        ...
        weighted_log_prob = self._estimate_log_prob(x).to(self.device) + torch.log(self.pi).to(self.device)
